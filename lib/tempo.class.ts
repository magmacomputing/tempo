...
import { Immutable, Serializable } from '#core/shared/decorator.library.js';
...
export const STORAGEKEY = '_Tempo_';												// for stash in persistent storage
 ...
 * - **Plugin System**: Extensible via "terms" to provide contextual date calculations (e.g., quarters, fiscal years).
 * 
 * @example
 * ```typescript
 * // Standard parsing
...
// #region Static enum properties~~~~~~~~~~~~~~~~~~~~~~~~~
	...
	/**
	 * {dt} is a laout that combines date-related {components} (dd, mm -or- evt) into a pattern against which a string can be tested.  
	...
	static #makeEvent(shape: Internal.Shape) {
		const events = ownEntries(shape.config.event)
			.map(([pat, _], idx) => `(?<evt${idx}>${pat})`)				// assign a number to the pattern
			.join('|')																						// make an 'Or' pattern for the event-keys
		shape.rules.component[Sym.evt] = new RegExp(events);		// set the component's 'event' pattern

		const date = Tempo.#isMonthDay(shape)										// set the {dt} layout (based on if we have a {locale} which prefers {mdy})
	...
	*/
	static #makePeriod(shape: Internal.Shape) {
		const periods = ownEntries(shape.config.period)
			.map(([pat, _], idx) => `(?<per${idx}>${pat})`)				// {pattern} is the 1st element of the tuple
			.join('|')																						// make an 'or' pattern for the period-keys
		shape.rules.component[Sym.per] = new RegExp(periods);		// set the components 'period' pattern
	...
	static #isMonthDay(shape: Internal.Shape) {
		const monthDay = asArray(Tempo.#global.config.monthDay)
			.concat(shape.config.level === 'global' ? [] : shape.config.monthDay)
		const shapeZone = shape.config.timeZone ?? Tempo.#global.config.timeZone;
		const shapeLocale = shape.config.locale ?? Tempo.#global.config.locale;
		console.log('monthDay: ', monthDay);
		console.log('shapeZone: ', shapeZone);
		console.log('shapeLocale: ', shapeLocale);
		let locale = monthDay																		// find a monthDay that contains our {timeZone}
			.find(itm => itm.timeZones?.includes(shapeZone))?.locale;// found an Intl.Locale which prefers {mdy} and contains our {timeZone}

		locale ??= monthDay																			// find a monthDay that contains our {locale}
			.find(itm => itm.locale === shapeLocale)?.locale;
		...
		options.forEach(option => {
			ownEntries(option)
				.forEach(([optKey, optVal]) => {
					...
					switch (optKey) {
						case 'layout':
							const obj = shape.rules["layout"];						// reference to the layout-record

							switch (arg.type) {
								case 'Object':															// add key-value pairs to Layout
									ownEntries(arg.value as Layout)
										.forEach(([key, val]) => obj[Tempo.getSymbol(shape, key)] = val);
									break;

								case 'String':															// add string with unique key to Layout
									obj[Tempo.getSymbol(shape)] = arg.value;
									break;

								case 'RegExp':															// add pattern with unique key to Layout
									obj[Tempo.getSymbol(shape)] = arg.value.source;
									break;
										...
										.forEach(elm => {
											...
											switch (itm.type) {
												case 'Object':
													...
													break;
												case 'String':
													obj[Tempo.getSymbol(shape)] = itm.value;
													break;
												case 'RegExp':
													obj[Tempo.getSymbol(shape)] = itm.value.toString();
													break;
											}
										})
									break;
	...
	static init(options: Tempo.Options = {}) {
		if (isEmpty(options)) {																	// if no options supplied, reset all
			...
			Tempo.#setConfig(Tempo.#global,
				{ version: VERSION, key: STORAGEKEY, component: Component, layout: Layout },
			...
			Tempo.#setConfig(Tempo.#global, Tempo.readStore());		// allow for storage-values to overwrite
		}
	 ...
	 */
	static readStore() {
		return getStorage<Tempo.Options>(Tempo.#global.config.key, {});
	}
	...
	static writeStore(config?: Tempo.Options) {
		return setStorage(Tempo.#global.config.key, config);
	}
	 ...
	 */
	until(opts?: Tempo.Options): Tempo.Duration;
	/** Calculates the duration until another date-time value. */
	until(dateTime: Tempo.DateTime, opts?: Tempo.Options): Tempo.Duration;
	...
	/** Calculates the duration until another date-time value in a specific unit. */
	until(until: Tempo.Until, opts?: Tempo.Options): number;
	/** Calculates the duration until another date-time value in a specific unit. */
	until(opts: Tempo.Options, until: Tempo.Until): number;
	until(until?: Tempo.DateTime | Tempo.Until, opts?: Tempo.Options | Tempo.Until) { return this.#until(until, opts) }
	 ...
	 */
	since(opts?: Tempo.Options): string;
	/** Calculates a human-readable string since another date-time. */
	since(dateTime: Tempo.DateTime, opts?: Tempo.Options): string;
	...
	/** Calculates a human-readable string since another date-time in a specific unit. */
	since(opts: Tempo.Options, until: Tempo.Until): string;
	since(since?: Tempo.DateTime | Tempo.Until, opts?: Tempo.Options | Tempo.Until) { return this.#since(since, opts) }
	...
	*/
	...
	#parseWeekday(groups: Internal.StringRecord, dateTime: Temporal.ZonedDateTime) {
		...
		if (isUndefined(wkd))																		// this is not a {wkd} pattern match
			return dateTime;
		 ...
		 * we early-exit if we find anything other than {wkd}, {mod}, {cnt} and time-units  
		*/
		const time = ['hh', 'mi', 'ss', 'ms', 'us', 'ns', 'ff', 'mer'] as const;
		const onlyTime = ownKeys(rest)
			.every(key => time.includes(key));										// only {time} keys are detected in {rest}
		if (!onlyTime)
			return dateTime;																			// this is not a true {wkd} pattern, so early-exit
