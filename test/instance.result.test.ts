...
describe(`${label} parse result accumulation`, () => {
  ...
  test('multiple .set() calls accumulate results', () => {
    ...
    expect(t3.parse.result.length).toBe(4); // 1 (constr) + 1 (ZonedDateTime from first set) + 2 (set time + ZonedDateTime) + 2 (set period + ZonedDateTime)
    // Actually each .set() adds 1 for the mutation + 1 for ZonedDateTime result
    // Let's adjust to be realistic:
